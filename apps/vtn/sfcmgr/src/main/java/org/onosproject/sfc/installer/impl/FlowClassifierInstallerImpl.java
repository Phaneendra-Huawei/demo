...
public class FlowClassifierInstallerImpl implements FlowClassifierInstallerService {
    ...
    */
    ...
    private PortNumber getPortFromMac(MacAddress mac, DeviceId deviceId) {
        PortNumber portNum = null;
        Device device = deviceService.getDevice(deviceId);
        List<Port> ports = new ArrayList<>(deviceService.getPorts(device.id()));
        for (Port port : ports) {
            String portMac =  port.annotations().value("portMac").toString();
            if (mac.equals(MacAddress.valueOf(portMac))) {
                portNum = port.number();
                break;
    ...
    public TrafficTreatment.Builder packTrafficTreatment(DeviceId deviceId, MacAddress srcMacAddress,
        ...
        if (host == null) {
            port = getPortFromMac(srcMacAddress, deviceIdfromPp);
        } else {
